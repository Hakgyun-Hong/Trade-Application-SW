...
class EBest:
    ...
    def __init__(self,mode=None):
        ...
        """
        config.ini 파일을 로드하고 사용자, 서버 정보 저장
        ...
        config = configparser.ConfigParser() # configparser 기능을 사용해서 conf폴더의 config.ini 내용을 불러옴.
        config.read('conf/config.ini') #config['섹션명']['옵션명']
        ...
        #XASession의 인스턴스를 생성.
        #해당 인스턴스는 login method 와 logout Method에서 사용된다.
        self.query_cnt=[]
    ...
    """
    login 메서드에서는 앞ㅇ서 구현한 XASession 객체를 이용하여 서버에 연결.로그인 시도.
    login_state의 값이 0일 동안 윈도우에서 발생하는 메시지를 보여주게 함.
    """
    ...
    def _execute_query(self,res,in_block_name,out_block_name,*out_fields,**set_fields):
        ...
        for item in result : 
            for field in list(item.keys()):
                if getattr(Field,res,None):# getattr 함수
    ...
    def get_stock_price_by_code(self,code=None,cnt="1"):
        ...
        #현재 result에는 요청한 종목의 코드가 없지만, Database에 저장하려면 코드가 필요.
        #따라서 for문을 이용해서 result의 개별 딕셔너리에 code값을 추가.
        for item in result:
            item["code"]=code
        return result

    def get_credit_trend_by_code(self, code=None,date=None): # 신용거래 동향.
        """
        TR: t1921 신용거래 동향
        :param code:str 종목코드
        ...
        out_params=["mmdate","close","sign","jcharge","diff","nvolume",
    ...
    def get_agent_trend_by_code(self, code=None ,fromdt=None, todt=None): #외인기관별 종목별 동향
    ...
    def get_short_trend_by_code(self, code=None, sdate=None, edate=None): # 공매도 일별 추이
        """TR: t1927 공매도일별추이
        :param code:str 종목코드
    ...
    def get_account_info(self):
        """
        계좌의 예수금, 주문가능금액, 총평가를 구하는 TR은 CSPAQ12200 이다.
        :return result:lsit Field CSPAQ12200 참고
        """
    ...
    def order_stock(self,code,qty,price,bns_type,order_type):
        ...
        :param order_type:str 호가유형
            00:자정가, 03:시장가 05:조건부지정가 07: 최우선지정가
            61:장개시전시간외 종가 81:시간외종가 82:시간외단일가
        :return result:dict 주문관련정보
        """
        in_params = {"AcntNo":self.account,"InptPwd":self.passwd,"IsuNo":code,"OrdQty":qty,"OrdPrc":price,"BnsTpCode":bns_type,"OrdprcPtnCode":order_type,
    ...
    def order_cancel(self,order_no,code,qty):
        ...
        out_params=["OrdNo","PrintOrdNo","OrdTime","OrdPtnCode","IsuNm"]
    ...
    def order_check(self,order_no=None):
        ...
        :param code:str 종목코드
        :param order_no:str 주문번호
        :param result:dict 주문번호의 체결상태
        """
        in_params={"accno":self.account,"passwd":self.passwd,"expcode":"", # code 는 어디서 쓰는거?
                    "chegb":"0","medosu":"0","sortgb":"1","cts_ordno":" "}
        out_params=["ordno","expcode","medosu","qty","price","cheqty","cheprice","ordrem","cfmqty","status","orgordno","ordgb","ordermtd",
                    "sysprocseq","hogagb","price1","orggb","singb","loandt"]
    ...
    def get_current_call_price_by_code(self,code=None):
        ...
        out_params=["hname","price","sign","change","diff","volume","jnilclose","offerho1","bidho1","offerrem1","bidrem1",
